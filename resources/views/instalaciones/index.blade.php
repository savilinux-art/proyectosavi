...
<div class="card"><div class="card-body">
    <table class="table table-striped" id="instalacionesTable">
        <thead><tr><th>ID</th><th>Proyecto</th><th>Instaladores</th><th>Inicio</th><th>Fin</th><th>Estatus</th><th>Acciones</th></tr></thead>
        <tbody>
            @foreach($instalaciones as $instalacion)
            <tr>
                <td>{{ $instalacion->id }}</td>
                <td>{{ $instalacion->proyecto->nombre_proyecto ?? 'N/A' }}</td>
                <td>@foreach($instalacion->instaladores as $inst)<span class="badge bg-primary">{{ $inst->nombre }}</span>@endforeach @if($instalacion->instaladores->isEmpty())<span class="text-muted">Sin asignar</span>@endif</td>
                ...
                <th>Ubicación Fin</th>
                    ...
                    <div class="btn-group">
                        ...
                        <form action="{{ route('instalaciones.destroy', $instalacion->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar esta instalación?')">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></button></form>
                    </div>
                </td>
            </tr>
            <td>

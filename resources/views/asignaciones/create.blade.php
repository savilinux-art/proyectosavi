...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('asignaciones.store') }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-md-6 mb-3"><label for="instalacion_id" class="form-label">Instalación *</label>
                <select class="form-select @error('instalacion_id') is-invalid @enderror" name="instalacion_id" id="instalacion_id" required>
                <option value="">Seleccionar</option>@foreach($instalacionesDisponibles as $i)<option value="{{ $i->id }}" {{ old('instalacion_id')==$i->id?'selected':'' }}>{{ $i->proyecto->nombre_proyecto ?? 'N/A' }} - {{ $i->estatus_instalacion ?? 'N/A' }}</option>@endforeach</select>@error('instalacion_id')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>

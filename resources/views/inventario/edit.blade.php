...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('inventario.update', $inventario->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6 mb-3"><label for="apea" class="form-label">APEA *</label><input type="text" class="form-control @error('apea') is-invalid @enderror" id="apea" name="apea" value="{{ old('apea', $inventario->apea) }}" required>@error('apea')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
